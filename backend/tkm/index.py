...
GOOGLE_FORM_ENTRIES = {
    ...
    # Раздел 1 — вопросы 2.1, 2.2, 2.3 обрабатываются отдельно по отделению (см. ниже)
    ...
    "3.22 Что такое ВПС?": "entry.715727214",
...
def send_to_google_forms(nickname: str, vk_link: str, department: str, activation_code: str, answers: dict):
    ...
    try:
        ...
        # Раздел 1 — entry ID для вопросов 2.1/2.2/2.3 зависят от отделения
        DEPT_ENTRIES = {
            "ОИК": {
            ...
            "СОП": {
                "2.1": "entry.783813198",
                "2.2": "entry.1788518830",
                "2.3": "entry.446046504",
            },
            "ОДС": {
                "2.1": "entry.1902761618",
                "2.2": "entry.1801011208",
                "2.3": "entry.1217621579",
            },
        }
        dept_entry_map = DEPT_ENTRIES.get(department, DEPT_ENTRIES["ОИК"])
        for q_key, entry_id in dept_entry_map.items():
            # Ищем ответ по частичному совпадению ключа
            for ans_key, ans_val in answers.items():
                if ans_key.startswith(q_key + " ") or ans_key == q_key:
                    form_data[entry_id] = str(ans_val)
                    break
        ...
        for key, value in answers.items():
            if key == "3.17":
                continue
            # Вопросы раздела 1 уже обработаны выше
            if key.startswith("2.1") or key.startswith("2.2") or key.startswith("2.3"):
                continue
